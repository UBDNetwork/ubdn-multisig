...
// check hold balance
contract DeTrustMultisigOnchainModel_00_a_05 is Test {
    ...
    function test_proxy1() public {
        ...
        vm.prank(address(1));
        // create and sign operation - not enough hold balance
        vm.expectRevert(
            abi.encodeWithSelector(FeeManager_01.LowHoldBalance.selector, modelReg.minHoldAmount(), 0)
        );
        uint256 lastNonce = multisig_instance.createAndSign(address(erc20), 0, _data);

        // add hold token balance for creator - cosigner[0] - not enough
        erc20Hold.transfer(address(1), modelReg.minHoldAmount() / 2);
        
        vm.prank(address(1));
        vm.expectRevert(
            abi.encodeWithSelector(FeeManager_01.LowHoldBalance.selector, 
                modelReg.minHoldAmount(), 
                modelReg.minHoldAmount() / 2)
        );
        lastNonce = multisig_instance.createAndSign(address(erc20), 0, _data);

        // add hold token balance for proxy
        erc20Hold.transfer(proxy, modelReg.minHoldAmount() / 2);
