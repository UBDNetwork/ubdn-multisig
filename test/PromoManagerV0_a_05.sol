...
contract PromoManagerV0_a_05 is Test {
    ...
    function setUp() public {
        ...
        PromoManagerV0.PromoPeriod memory promoData = PromoManagerV0.PromoPeriod(validTill, promoPeriodP);
        ...
        promoData = PromoManagerV0.PromoPeriod(validTill, promoPeriodC);
        promoManager.setPromoPeriod(promoHash, promoData);
    }

    // there are fee, hold to creation, hold to using
    // private promo is active - deploy proxy and make action
    // only pay deploy fee
    function test_promo_1() public {
        ...
        assertEq(infoFee.fee.payedTill, currentTime + multisig_instance.ANNUAL_FEE_PERIOD() + promoPeriodP); // + 2 * 365 day of promo
        assertEq(infoFee.freeHoldPeriod, currentTime + promoPeriodP); // + 2 * 365 day of promo
        ...
        // check:  hold Condition  is free by promo reason
